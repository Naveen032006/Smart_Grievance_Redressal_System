...
const getMyWardAdmin = async (req, res) => {
  try {
    ...
    if (!userWardNumber) {
        return res.status(400).json({ success: false, message: "User is not assigned to a ward." });
    }
    const admin = await adminModel.findOne({ wardNumber: userWardNumber }).select('-password');
    if (!admin) {
        return res.status(404).json({ success: false, message: "No admin found for this ward." });
    }
    res.json({ success: true, data: admin });
  } catch (error) {
    console.log(error);
    res.status(500).json({ success: false, message: "Error fetching ward admin" });
  }
};
...
const getMyWardEmployees = async (req, res) => {
  try {
    ...
    console.log(error);
    res.status(500).json({ success: false, message: "Error fetching ward employees" });
  }
};
...
const getMyWardStats = async (req, res) => {
    try {
        ...
        console.log(error);
        res.status(500).json({ success: false, message: "Error fetching ward stats" });
    }
};
