...
export const Mycomplainwrap = ({
  ...
  useEffect(() => {
    const url = role === "user" ? "/user/my-issues" : "/admin/issues";
    ...
    fetchComplaints();
  }, [role]); // Re-fetch if the user type changes
        ...
        filteredComplaints.map((issue) => (
          <div key={issue._id} style={styles.complaintBox}>
            ...
            <div style={styles.details}>
              <h4>
                {issue.issueTitle} (Status: {issue.status})
              </h4>
              <p>Category: {issue.category}</p>
              <p>Location: {issue.location}</p>
              {role === "Admin" && <p>Submitted by: {issue.user.userid}</p>}
              <p>
